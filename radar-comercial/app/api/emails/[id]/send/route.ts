...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: d } = await db().from('radar_email_drafts').select('*').eq('id', id).single();
  ...
  const { count: sentToday } = await db().from('radar_email_drafts').select('id', { count: 'exact', head: true })
  ...
  if (!force) {
    ...
    const { data: recent } = await db().from('radar_email_drafts').select('sent_at').eq('city_id', d.city_id)
  ...
  try {
    ...
    await db().from('radar_email_drafts').update({ status: 'enviado', sent_at: new Date().toISOString(), message_id: messageId, error: null }).eq('id', id);
    if (d.opportunity_ids?.length) {
      await db().from('radar_opportunities').update({ status: 'em_andamento' }).in('id', d.opportunity_ids).eq('status', 'novo');
    }
    return NextResponse.json({ ok: true });
  } catch (e: any) {
    await db().from('radar_email_drafts').update({ error: String(e?.message || e).slice(0, 500) }).eq('id', id);
