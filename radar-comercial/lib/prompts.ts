...
1. Orçamento: você tem até ${maxSearches} buscas — cada uma tem custo, use com critério. Planeje antes de buscar: 2-3 consultas amplas que cubram vários sinais de uma vez, depois aprofunde só os achados promissores. Pare assim que o sinal se esgotar (resultados repetidos ou sem novidade) — não force usar todas as buscas. Mas também não desista cedo demais: se as primeiras 1-2 buscas vierem vazias, tente ao menos uma ou duas reformulações diferentes (nome de bairro suspeito, "prefeitura de X reintegração de posse", nome do rio local + enchente) antes de declarar que não há nada, especialmente para os sinais de invasão/APP/enchente da seção abaixo. Nunca repita consultas iguais ou quase iguais.
